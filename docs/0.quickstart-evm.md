...
::banner{type="note"}
Want to skip a tutorial? Choose `EVM-compatible` and `demo_evm_logs` instead!
::
...
Now it's time to generate typeclasses and callback stubs based on definitions from config. Examples below use `demo_evm_logs` as a package name; yours may differ.
...
DipDup will create a Python package `demo_evm_logs` with everything you need to start writing your indexer. Use `package tree` command to see the generated structure:
...
{{ #include ../src/demo_evm_logs/models/__init__.py }}
```
...
{{ #include ../src/demo_evm_logs/handlers/on_transfer.py }}
```
...
```bash
sqlite3 demo_evm_logs.sqlite 'SELECT * FROM holder LIMIT 10'
```
