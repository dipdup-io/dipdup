...
| `ignore`              | Ignore the event and continue indexing as usual. It can lead to unexpected side-effects up to data corruption; make sure you know what you are doing. A `rollback` is skipped, and the flag stays until `dipdup schema approve`.       |
