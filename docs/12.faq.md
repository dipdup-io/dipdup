...
You may want to disable schema hash check in config. Alternatively, call the `schema approve` command after every schema change.
...
Now, let's prepare a migration script. To determine changes you need to make, you can compare the SQL schema dump before and after modifying the models. Say you need to add a new field to one of models.
...
Now prepare the idempodent migration script and put to the `sql/on_restart` directory.
