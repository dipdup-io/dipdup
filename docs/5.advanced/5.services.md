...
If your new command is a long-running service, you need to update Dockerfile and Compose manifests. Update the `deploy/Dockerfile` to use the new entrypoint:
...
::banner{type="warning"}
Do not run multiple DipDup projects with the same database configuration! It _will_ lead to schema conflicts and data corruption.
::

The new project directory can be either a sibling of the main project or a subdirectory. In this example, we will create a nested package `{{ project.package }}/service`
...
::banner{type="info"}
Starting with DipDup 8.5, you can run multiple DipDup projects on the same database, but different schemas.
...
# NOTE: Hasura integration currently requires `public` schema to be used
hasura: null
::
