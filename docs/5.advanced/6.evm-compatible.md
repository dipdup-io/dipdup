...
EVM indexes can use three datasources:

### [evm.subsquid](../3.datasources/4.evm_subsquid.md)

Subsquid Network is where DipDup gets historical data from. **It's the only datasource required to operate.**
...
Etherscan API is a source of contract ABIs, which are used to generate types for the indexer.
...
EVM node RPC can be used to fetch recent data not yet in Subsquid Network. API methods could vary a lot across different networks, but DipDup only uses a few of them, so most of the nodes will work.
