...
After maybe-too-long awaited 7.0 stable release we decided to establish shorter and more predictable release cycle. So, expect new DipDup versions to be less groundbreaking and more iterative. Today we're releasing DipDup 7.1 with new index kind, CLI options, and lots of small improvements. Let's dive in!

## Indexing Tezos token balanses

Starting with 7.1 new index kind is available, `tezos.tzkt.token_balances`.
