...
* DipDup utilizes a microservice approach and relies heavily on existing solutions, which makes the SDK itself very lightweight and allows to switch API engines on demand.
...
DipDup needs to handle chain reorgs since reindexing from scratch leads to several minutes of downtime. Single level rollbacks are now processed in the following way:
