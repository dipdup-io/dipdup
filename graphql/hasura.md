...
Keep in mind that "camelcasing" is a separate stage performed after all tables are registered. So during configuration, you can observe fields in `pascal_case` for several seconds even if `hasura.camel_case` flag is set. 
