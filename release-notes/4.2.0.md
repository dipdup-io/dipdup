...
Likewise `metadata`, datasource HTTP connection parameters (ratelimit, backoff, and so on) are applied on every request.
...
Cookiecutter is a cool `jinja2` wrapper to interactively initialize hello-world templates of various frameworks and toolkits. Install `python-cookiecutter` package systemwide, then call:
...
DipDup utilizes `apscheduler` library to run hooks according to schedules in `jobs` config section. In the following example `apscheduler` will spawn up to three instances of the same job every time trigger is fired, even if previous runs are in progress:
...
Note that you can't use executors from `apscheduler.executors.pool` module - `ConfigurationError` exception is raised. If you're into multiprocessing, I'll explain why in the next paragraph.

### About the present and future of multiprocessing

It's not possible to use `apscheduler` pool executors with hooks because HookContext is not pickle-serializable. So, they are forbidden now in `advanced.scheduler` config. However, thread/process pools are can come in handy in many situations. It would be nice to have them in DipDup context. For now, I can suggest implementing custom commands as a workaround, and perform any resource-hungry tasks within them. Put the following code in `<project>/cli.py`:
