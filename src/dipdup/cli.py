...
async def run(
    ...
    if postpone_jobs:
        _logger.warning('`--postpone-jobs` {}'.format(warn_text))
        config.advanced.postpone_jobs |= postpone_jobs
    if early_realtime:
        _logger.warning('`--early-realtime` {}'.format(warn_text))
        config.advanced.early_realtime |= early_realtime
    if merge_subscriptions:
        _logger.warning('`--merge-subscriptions` {}'.format(warn_text))
        config.advanced.merge_subscriptions |= merge_subscriptions
    if metadata_interface:
        _logger.warning('`--metadata-interface` {}'.format(warn_text))
