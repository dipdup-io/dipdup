...
SETTINGS_JSON = {
    'yaml.schemas': {
        'schema.json': ['**/dipdup*.y[a]ml', 'tests/configs/*.yml'],
    },
    'files.associations': {'*.env.default': 'shellscript'},
}
...
def answers_from_terminal(template: str | None) -> Answers:
    ...
    answers['license'] = survey.routines.input(
        'Enter project license (DipDup is under MIT): ',
    ...
    _, answers['vscode'] = prompt_anyof(  # type: ignore[typeddict-item]
        question='Enable VSCode integration?',
...
def render_project(
    ...
    if answers['vscode']:
        _render_vscode(Path(answers['package']) / '.vscode', force)
...
def _render_vscode(
    ...
    settings['yaml.schemas'].update(SETTINGS_JSON['yaml.schemas'])
    if 'files.associations' not in settings:
        settings['files.associations'] = {}
    settings['files.associations'].update(SETTINGS_JSON['files.associations'])

    if not settings_path.exists() or force:
        settings_path.write_text(orjson.dumps(settings, option=orjson.OPT_INDENT_2).decode())

    if not schema_path.exists() or force:
        schema_dict = DipDupConfig.json_schema()
        schema_path.write_text(orjson.dumps(schema_dict, option=orjson.OPT_INDENT_2).decode())
