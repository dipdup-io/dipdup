...
async def test_in_global_transaction() -> None:
    ...
    async with tortoise_wrapper('sqlite://:memory:'):
        ...
        await Index(name='1', type=IndexType.tezos_tzkt_operation, config_hash='').save()
        count = await Index.filter().count()
        assert count == 1

        # 2. Success query within transaction
        async with transactions.in_transaction():
            await Index(name='2', type=IndexType.tezos_tzkt_operation, config_hash='').save()
        count = await Index.filter().count()
        assert count == 2

        # 3. Not rolled back query without transaction
        with suppress(SomeException):
            await Index(name='3', type=IndexType.tezos_tzkt_operation, config_hash='').save()
            raise SomeException
        count = await Index.filter().count()
        assert count == 3
        ...
        with suppress(SomeException):
            async with transactions.in_transaction():
                await Index(name='4', type=IndexType.tezos_tzkt_operation, config_hash='').save()
                raise SomeException
        count = await Index.filter().count()
        assert count == 3
